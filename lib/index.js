 ...
 * ### Examples:
 *
 *     remote.remote('user', 'repo', function(err, remote) {
 *       remote.copy('.', 'vendors/user-repo');
 *     });
 *
 *     remote.remote('user', 'repo', 'branch', function(err, remote) {
 *       remote.copy('.', 'vendors/user-repo');
 *     });
 *
 *     remote.remote('http://foo.com/bar.zip', function(err, remote) {
 *       remote.copy('.', 'vendors/user-repo');
 *     });
 *
...
remote.cacheRoot = function cacheRoot() {
  return path.join(xdgBasedir.cache, 'node-fetch-remote-cache');
};
 ...
 * using `remote.remote()` (eg such as `node_modules` installed via `package.json`)
 * @param  {String}   cache
 * @param  {Function} cb
 * @example
 *  ### Examples
 *
 *     remote.remoteDir('foo/bar', function(err, remote) {
 *       remote.copy('.', 'vendors/user-repo');
 *     });
 */
